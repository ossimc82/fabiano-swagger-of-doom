    ...
    public class QrCodeGenerator
    {
        ...
        private static class ModulePlacer
        {
            ...
            public static int MaskCode(ref QrCode qrCode, int version, ref List<Rectangle> blockedModules)
            {
                ...
                foreach (var pattern in typeof(MaskPattern).GetMethods())
                {
                    ...
                    if (!string.IsNullOrEmpty(patternName) && patternScore <= score) continue;
            ...
            public static void PlaceAlignmentPatterns(ref QrCode qrCode, IEnumerable<Point> alignmentPatternLocations, ref List<Rectangle> blockedModules)
            {
            ...
            private static bool IsBlocked(Rectangle r1, IEnumerable<Rectangle> blockedModules)
            {
                var isBlocked = false;
                foreach (var blockedMod in blockedModules.Where(blockedMod => Intersects(blockedMod, r1)))
                    isBlocked = true;
                return isBlocked;
            }
            ...
            private static class MaskPattern
            {
                ...
                public static int Score(ref QrCode qrCode)
                {
                    ...
                    //Penalty 4
                    var blackModules = qrCode.ModuleMatrix.SelectMany(row => row.Cast<bool>()).Count(bit => bit);
        ...
        private void CreateAlignmentPatternTable()
        {
            ...
            for (var i = 0; i < (7 * 40); i = i + 7)
            {
                ...
                for (var x = 0; x < 7; x++)
                {
                    if (alignmentPatternBaseValues[i + x] == 0) continue;
                    for (var y = 0; y < 7; y++)
                    {
                        if (alignmentPatternBaseValues[i + y] == 0) continue;
